      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-12">
          {t.experience.jobs.map((job, index) => (
            <div key={index} className="relative pl-8 border-l-2 border-dark-800">
              <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-primary-500 border-4 border-dark-950"></div>
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-dark-100">{job.role}</h3>
                <p className="text-primary-400 font-mono text-sm">{job.company}</p>
                <p className="text-dark-500 text-sm mt-1">{job.period}</p>
              </div>
              <ul className="space-y-3">
                {job.achievements.map((achievement, i) => (
                  <li key={i} className="text-dark-300 text-sm flex items-start gap-3">
